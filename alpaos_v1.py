...
#@jit(nopython=True)
def laplacianSolver(H, K, outside_neighbours, boundary_i, boundary_j, outside_mask, chan, max_iterations=100000, min_iterations = 500):
    ...
    outside_neighbours  = outside_neighbours.astype(np.int32)
    chan                = chan.astype(np.int32)
    boundary_i          = boundary_i.astype(np.int32)
    boundary_j          = boundary_j.astype(np.int32)
    outside_mask        = np.asarray(outside_mask, dtype = np.int64)
    ...
    while continue_flag:
        ...
        H_maxs.append(np.nanmax(H))

        counter += 1
        #print('iteration: ', counter, 'max H: ', np.nanmax(H), end='\r')

        if counter >= max_iterations:
            continue_flag = False
        if len(H_maxs) > min_iterations:
            if (np.nanmax(((H - H_prev)**2)**.5)) < 0.001:
                        continue_flag = False
            else:
                H_prev = H.copy()

    return H, np.asarray(H_maxs), H_prev, counter
...
class AlpaosChannelCreator:
    ...
    def solve(self, iterations = 100, gamma = 9810, tau_crit = 0.001, max_iterations = 100000, H_min_iterations = [100,10], T = "Hmean"):
            ...
            for iter in range(iterations):
                time0                       = time.time()
                lapl_result                 = laplacianSolver(H = self.H, K = self.K, outside_neighbours = self.outside_neighbours,
                                                                boundary_i = self.boundary_i, boundary_j = self.boundary_j,
                                                                outside_mask = self.outside_mask, chan = self.chan,
                                                                max_iterations = max_iterations, min_iterations = H_min_iterations[min(iter,1)])
                self.H, self.H_maxs, self.H_prev, counter = lapl_result
                time1                       = time.time()

                print(f"Step: {iter+1}/{iterations}")
                if time1 - time0 > 60:
                    print(f'Solving Poisson equation for H took {np.round((time1-time0)/60,2)} minutes to run {counter - 1} iterations.')
